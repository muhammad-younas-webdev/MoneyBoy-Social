...
(function () {
  ...
  if (swiperContainers.length > 0) {
    console.log(
      `Found ${swiperContainers.length} Swiper container(s). Loading Swiper CDN.`
    );
    ...
    script.onerror = () => {
      console.error(
        "Failed to load Swiper.js CDN. Swiper functionality may be broken."
      );
    };
  } else {
  ...
  function initializeSwipers(containers) {
    ...
    containers.forEach((wrapper) => {
      ...
      const prevButton = wrapper.querySelector(
        ".moneyboy-swiper-control-btn.prev-btn"
      );
      const nextButton = wrapper.querySelector(
        ".moneyboy-swiper-control-btn.next-btn"
      );

      // Safety check
      if (!swiperElement || !prevButton || !nextButton) {
        console.warn(
          "Skipping Swiper: Missing core elements (swiper, prev, or next button) inside wrapper.",
          wrapper
        );
        return;
      }
      ...
      try {
        ...
        console.log(
          "Initialized Swiper successfully for element:",
          swiperElement
        );
